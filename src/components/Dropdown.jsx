  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-expanded={open}
        className="flex w-full items-center justify-between gap-6 rounded-full border border-noir/15 bg-transparent px-5 py-3 font-sans text-sm text-noir transition-colors duration-300 hover:border-noir/35"
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            className={`absolute z-30 mt-2 max-h-72 min-w-[15rem] overflow-auto rounded-xl border border-noir/10 bg-canvas p-1.5 shadow-[0_24px_60px_-30px_rgba(24,19,16,0.5)] ${
                ...
                <li key={o.value}>
                  <button
                    ...
                    className={`flex w-full items-center justify-between rounded-lg px-4 py-2.5 text-left font-sans text-sm transition-colors duration-200 ${
                      active ? 'bg-gold text-noir' : 'text-noir/80 hover:bg-canvas-deep'
                    }`}
                  >
                    {o.label}
                    {active && (
