...
export function formatPrice(value, { rent = false } = {}) {
  if (value == null) return 'On application'
  if (rent) return `AED ${value.toLocaleString('en-US')}/yr`
  if (value >= 1_000_000) {
...
export function formatPriceFull(value, { rent = false } = {}) {
  if (value == null) return 'Price on application'
  return rent ? `AED ${value.toLocaleString('en-US')} / year` : `AED ${value.toLocaleString('en-US')}`
}
